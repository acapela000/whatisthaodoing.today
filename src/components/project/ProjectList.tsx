...
export function ProjectList() {
  ...
  const pages: number = Math.ceil(projectList.length / projectsPerPage);
  const [] = useState('');
  ...
  const [projectEnd, setProjectEnd] = useState(projectsPerPage);
  ...
  const next = () => {
    if (currentPage === pages) {
      setProjectEnd(projectList.length - 1)
      return
    };
  ...
  // const tmplist = [1...pages].map((el) => <IconButton {...getItemProps(el)}>{el}</IconButton>)
  function getButtonNumberListTSX() {
    let buttonNumberListTSX = [];

    for (let pageNumber = 1; pageNumber <= pages; pageNumber++) {
      buttonNumberListTSX.push(
        <IconButton {...getItemProps(pageNumber)}>{pageNumber}</IconButton>
      )
    }

    return buttonNumberListTSX;
  }
    ...
    <div className="mt-20">
      ...
      <div className="grid xs:grid-cols-1 md:grid-cols-2 gap-4 my-5 ">
        {projectList.slice(projectStart, projectEnd).map((el) => {
          return (
            <div key={el.name} className="max-w-sm shadow shadow-blue-500/40 hover:shadow-indigo-500/40 bg-white border border-gray-200 rounded-lg dark:bg-gray-800 dark:border-gray-700">
              <a href={el.github}>
                <img className="rounded-t-lg h-48 w-full grayscale hover:grayscale-0 duration-700 object-cover" src={el.thumbnail} alt="img" />
              </a>
              <div className="p-5">
                <h5 className="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white hover:text-gray-900 hover:dark:text-white">{el.name}</h5>
                <p className="line-clamp-5 mb-3 font-normal text-gray-700 dark:text-gray-400 project-description" dangerouslySetInnerHTML={{ __html: el.description }} />
