      ...
      <div className="grid grid-cols-3 gap-1 justify-items-center my-5">
        {projectList.slice(projectStart, projectEnd).map((el) => {
          return (
            <div
              key={el.name}
              className="max-w-sm shadow shadow-blue-500/40 hover:shadow-indigo-500/40 bg-white border border-gray-200 rounded-lg dark:bg-gray-800 dark:border-gray-700"
            >
              <a href={el.github}>
                <img
                  className="rounded-t-lg h-48 w-full grayscale hover:grayscale-0 duration-700 object-cover"
                  src={el.thumbnail}
                  alt="img"
                />
              </a>
              <div className="p-5">
                <h5 className="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white hover:text-gray-900 hover:dark:text-white">
                  {el.name}
                </h5>
                <p
                  className="line-clamp-5 mb-3 font-normal text-gray-700 dark:text-gray-400 project-description"
                  dangerouslySetInnerHTML={{ __html: el.description }}
                />
                <a
                  href={el.github}
                  target="_blank"
                  className="inline-flex items-center px-3 py-2 text-sm font-medium text-center text-black focus:ring-4 focus:outline-none"
                >
                  <svg
                ...
                {el.url && (
                  ...
                  >
                    <svg
                      ...
                      viewBox="0 0 128 128"
                    >
                      <circle fill="" cx="63.624" cy="64.474" r="22.634" />
                      <path
                        fill=""
